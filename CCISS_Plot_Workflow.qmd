...
predict_bgc(con, dem_table, BGCmodel, vars_needed, gcms_cciss, periods_use = c("2001_2020"), max_runs_use = 4L)
...
spps <- unique(S1$spp)
spps.candidate <- T1$TreeCode[-which(T1$Exclude=="x")]
...
```{r}
spp_bubbleplot(perexp, period = "2041_2060", scenario = "ssp370", edatope = "C4")

spp_bubbleplot(perexp, period = "2041_2060", scenario = "ssp245", species = "Pl", edatope = c("B2", "C4", "D6"), by = "edatope")
```
...
```{r}
spp_spaghettiplot(sa, species = "Cw", use_MAT = FALSE)
...
plot_SuitabilityChangeMap(con, bgc_template = bgc_template, outline = bc_ol, spp = "Fd", edatope = "B2", period = "2041_2060", three_panel = FALSE, save_png = FALSE)
```
