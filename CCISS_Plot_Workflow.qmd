...
```{r}
con <- dbCon_cciss("./BuMoTest.duckdb", threads = 8)
...
```{r}
sa <- spp_suit_area(con, spp_list = c("Pl","Sx","Lw"))
```

## Species Spaghetti Plot with indiv. Runs

```{r}
spp_spaghettiplot(sa, species = "Pl")
```
...
```{r}
plot_spparea(spp = "Fd", edatope = "B2", bgc_template = bgc_template, fractional = TRUE, base_folder = base_path)
plot_spparea(spp = "Fd", edatope = "C4", bgc_template = bgc_template, fractional = TRUE, base_folder = base_path)
plot_spparea(spp = "Fd", edatope = "D6", bgc_template = bgc_template, fractional = TRUE, base_folder = base_path)
```

Let's try a different species:
