...
BGCmodel <- readRDS(here("tree_summaries_code", "BGCmodel_WNA_V2.1.rds"))
...
#DBI::dbExecute(con, "PRAGMA max_temp_directory_size='18GiB'") #Mia added 
...
#Tweaks I'd like to make here:
#   1) Customize color palette, maybe temps getting hotter?
#   2) Make spaghettini higher alpha, so that mean lines more prominent
#   3) More descriptive legend title; "shared socioeconomic pathways (SSPs)"
```

Save these plots to a subfolder, so they can be called into the dashboard qmd scripts:

```{r}
ggsave("Cw_spag.png", plot = Cw_spag, path = here("tree_summaries_code", "Mia", "Figures"),width = 8, height = 4, dpi = 300)
```
...
#   1) y axis labels
#   2) increase alpha of transparent rectangles
#   3) Show only the legend for BGC colors that the species is actually present in. It's showing all of them always now, right?
