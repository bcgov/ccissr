...
png("Figures/Cw_ssp370_bub.png", width = 1800, height = 1200, res = 200)
spp_bubbleplot(perexp, period_sel="2081_2100", scenario="ssp370", #ssp370 worst, 245 moderate, 126 optimistic
               species="Cw", edatope=c("B2","C4","D6"), by="edatope")
dev.off()
```
...
sa <- spp_suit_area(con, spp_list = c("Pl")) #try only 1 spp at a time
```

## Species Spaghetti Plot with indiv. Runs

```{r}
Pl_spag <- spp_spaghettiplot(sa, species = "Pl", use_MAT = FALSE); Pl_spag

Pl_spag2 <- spp_spaghettiplot(sa, species = "Pl", use_MAT = TRUE); Pl_spag2
...
```{r}
ggsave("Pl_spag.png", plot = Pl_spag, path = here("tree_summaries_code", "Figures"),width = 8, height = 4, dpi = 300)

ggsave("Pl_spag2.png", plot = Pl_spag2, path = here("tree_summaries_code", "Figures"),width = 8, height = 4, dpi = 300)
```
...
Pl_B2_alluv <- plot_spparea(con, spp = "Pl", edatope = "B2", fractional = TRUE); Pl_B2_alluv
Pl_C4_alluv <- plot_spparea(con, spp = "Pl", edatope = "C4", fractional = TRUE); Pl_C4_alluv
Pl_D6_alluv <- plot_spparea(con, spp = "Pl", edatope = "D6", fractional = TRUE); Pl_D6_alluv

#Mods: 
#rectangles are super incorrect - commented them out for now
#new zones are slightly offset to right of bar, compared to pre-existing colors. 
#change white rectangles back to higher alpha...try 8.5.
```

Save these plots to a subfolder, so they can be called into the dashboard qmd scripts:

```{r}
ggsave("Pl_B2_alluv.png", plot = Pl_B2_alluv, path = here("tree_summaries_code", "Figures"),width = 6, height = 4, dpi = 300)

ggsave("Pl_C4_alluv.png", plot = Pl_C4_alluv, path = here("tree_summaries_code", "Figures"),width = 6, height = 4, dpi = 300)

ggsave("Pl_D6_alluv.png", plot = Pl_D6_alluv, path = here("tree_summaries_code", "Figures"),width = 6, height = 4, dpi = 300)
...
# I was getting an error running the latest version of the function from cciss_analysis.R, and decided to modify it in a new script so as to not break anything for anyone else. I then also made stand-alone plots of the historical suitability maps, and a new 2-panel plot of just the future mean change map+boxplot. 

# --> Go to 2-panel_updates.R in ccissr>tree_summaries_code
```
