...
for(proj.year in proj.years){
 ...
 text(xr+40000, mean(c(yb,yt))-30000, paste("Relative feasibility richness"), srt=90, pos=3, cex=1, font=2)
 ...
 text(-.025, 0.65, "Historical feasibilities", pos=4, cex=0.8, font=2, offset=0.1)
 text(0, 0.525, std.ref, pos=4, cex=0.8, offset=0.1)
 text(-.025, 0.35, paste("Mean projected feasibilities", sep=""), pos=4, cex=0.8, font=2, offset=0.1)
 text(0, 0.225, std.proj, pos=4, cex=0.8, offset=0.1)
 text(-.025, 0.05, paste("Mean rel. feas. richness: ", round(get(metric)[pt]*100), "%", sep=""), pos=4, cex=0.8, offset=0.1, font=2)
 text(0.605, .95, paste("Projected bioclimates"), pos=4, cex=0.8, font=2, offset=0.1)

  par(mar=c(3,0,1,0), plt = c(0.85, 0.985, 0.10, 0.165), new = TRUE, mgp=c(0.5,0.1,0))
 ...
 bxp(z, add=T, boxfill = as.character(BGCcolors$colour[match(levels(droplevels(zone[-grep("BAFA|CMA|IMA",zone)])), BGCcolors$classification)]), xaxt="n", yaxt="n", xaxs="i", ylab="Relative feasibility richness", pch=0,outline=FALSE)
...
for(edatope in edatopes){
   ...
   for(proj.year in proj.years[1:2]){
      ...
      SuitRichnessChangePct.truncate[!is.finite(SuitRichnessChangePct.truncate)] <- NA
            ...
            # x11(width=6.5, height=5, pointsize=8)
            png(filename=paste("results\\Manu_Richness\\CCISS_", metric,"_", rcp,"_", proj.year,"_", edatope,".png",sep=""), type="cairo", units="in", width=6.5, height=5.5, pointsize=11, res=400)
            # pdf(file=paste("results\\CCISS_SummaryByBGC_", metric,".pdf",sep=""),  width=7.5, height=5.625, pointsize=15)
...
#    text(-.025, 0.75, "Historical feasibilities", pos=4, cex=0.8, font=2, offset=0.1)
#    text(0, 0.65, std.ref, pos=4, cex=0.8, offset=0.1)
#    text(-.025, 0.55, paste("Mean projected feasibilities", sep=""), pos=4, cex=0.8, font=2, offset=0.1)
