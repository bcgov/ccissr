...
# base raster
# X <- raster("//objectstore2.nrs.bcgov/ffec/DEM/BC_Raster.tif") #this is the permanent primary location, but read from a local copy as reading from object storage slows the script down. 
...
for(timeperiod in timeperiods[-1]){
  ...
  for(eda in edas){
    ...
    for(spp in spps){ ##ignore warnings,"Fd","Sx","Pl", "Yc", "Yc", "Oa", "Yp"
      ...
      #legend. note that climr::legend_ramp() can do the elements of this legend, but not the complex full legend. 
      ...
      ##=================================
      ## d. Summary by zone (has to come before panel c because the boxplots are summarizing values of panel b)
