---
title: "CCISS Spatial Workflow (datatable version)"
author: "Kiri Daust"
format: html
editor: visual
---
...
We'll use the high resolution dem for the lower mainland. It's small enough that we can run the analysis quickly. To demonstrate the data.table workflow, we'll project it to BC Albers, and then extract the cell coordinates.
...
points_in <- as.data.frame(dem, xy = TRUE, na.rm = TRUE, cells = TRUE)
coords_4326 <- project(as.matrix(points_in[,c("x","y")]), from = "epsg:3005", to= "epsg:4326")
points <- data.table(id = points_in$cell,lon = coords_4326[,1], lat = coords_4326[,2], elev = points_in$WNA_DEM_SRT_30m)
```
...
```{r}
summary_preds_gcm(points, BGCmodel, vars_needed, gcms_cciss, periods_use = periods, base_folder = base_path)
...
pts_bgc <- make_bgc_template(points, bgcs)
```

Looks good. Now we can pass that to the edatopic overlap function.

```{r}
siteseries_preds(edatopes = eda, bgc_mapped = pts_bgc, base_folder = base_path)
```
...
```{r}
cciss_2041 <- fread(file.path(base_path,"cciss_suit","CCISS_2041_2060_C4.csv"))
cw_2041 <- cciss_2041[Spp == "Cw",]
cw_rast <- copy(dem)
values(cw_rast) <- NA
cw_rast[cw_2041$SiteRef] <- cw_2041$Newsuit
plot(cw_rast)
```
